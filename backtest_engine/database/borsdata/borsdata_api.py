...
class BorsdataAPI:
    def __init__(self, _api_key):
        ...
        self._params = {'authKey': self._api_key, 'maxYearCount': 20, 'maxR12QCount': 40, 'maxCount': 40}
    ...
    def get_kpi_history(self, ins_id, kpi_id, report_type, price_type, max_count=None):
        ...
        df.set_index(["year", "period"], inplace=True)
    ...
    def get_kpi_summary(self, ins_id, report_type, max_count=None):
        ...
        df.rename(
            columns={"q": "quarter", "y": "year", "p": "period", "v": "kpiValue", "KpiId": "kpiId"},
            inplace=True,
        )
        df = df.pivot_table(
            index=["quarter", "period"], columns="kpiId", values="kpiValue"
        )
        self._set_index(df, ["quarter", "period"], ascending=False)
        return df
    ...
    def get_instrument_report(self, ins_id, report_type, max_count=None):
        ...
        self._set_index(df, ["year", "period"], ascending=True)
        return df
...
if __name__ == "__main__":
        ...
        :return: pd.DataFrame
        """
        api = BorsdataAPI('892e4024330946aab65c7c09983cf3ec')
        val = api.get_kpi_data_instrument(ins_id=252,kpi_id=148,calc_group='last',calc='latest')
        prices = api.get_instrument_stock_prices(ins_id=252,from_date='2022-10-17',to_date='2023-10-17',max_count=300)
        print(prices)